...
export type PaymentPlanRecommendationImpact =
  | 'lowerTotalPayment'
  | 'lowerInitialInstallment'
  | 'balancedInstallment'
  | 'paymentStructure';

export type PaymentPlanRecommendation = {
  id: string;
  planType: LoanPlanType;
  title: string;
  message: string;
  details: string[];
  impact: PaymentPlanRecommendationImpact;
  comparison: {
    totalPaymentDifference: number;
    totalInterestDifference: number;
    firstInstallmentDifference: number;
    firstYearAverageDifference: number;
  };
};
...
type RankedRecommendation = PaymentPlanRecommendation & {
  score: number;
};
...
const PLAN_LABELS: Record<LoanPlanType, string> = {
  standard: 'Standart Sabit Taksitli Plan',
  prepaidInterest: 'Peşin Faiz Ödemeli Plan',
  equalPrincipal: 'Eşit Anapara Planı',
...
const buildComparableBaseInput = (input: LoanInput): LoanInput => ({
...
const calculateCandidate = (
  ...
  try {
    const candidateInput: LoanInput = {
      ...baseInput,
      planType,
    };
    ...
    if (
      ...
      candidateInput.installmentIncreaseRatePercent =
        DEFAULT_PROGRESSIVE_RATE_PERCENT;
...
const averageInstallment = (
  ...
  const total = rows.reduce(
    (sum, item) => roundToCents(sum + item.installment),
    0
  );
...
const getFirstRegularInstallment = (result: LoanCalculationResult): number =>
  result.schedule.find((item) => !item.isPrepaidInterest)?.installment ?? 0;

const getLastRegularInstallment = (result: LoanCalculationResult): number => {
  const regularRows = result.schedule.filter((item) => !item.isPrepaidInterest);

  return regularRows[regularRows.length - 1]?.installment ?? 0;
};

const isMeaningfulAmount = (value: number): boolean =>
  Math.abs(value) >= MIN_AMOUNT_DIFFERENCE;

const buildComparison = (
  baseline: LoanCalculationResult,
  alternative: LoanCalculationResult
): PaymentPlanRecommendation['comparison'] => ({
  totalPaymentDifference: roundToCents(
    alternative.totalPayment - baseline.totalPayment
  ),
  totalInterestDifference: roundToCents(
    alternative.totalInterest - baseline.totalInterest
  ),
...
const buildTotalPaymentText = (difference: number): string => {
  if (difference < 0) {
...
const buildProgressiveAssumptionText = (
  alternative: LoanCalculationResult
): string | undefined => {
  if (
    alternative.planType !== 'increasingInstallment' &&
    alternative.planType !== 'decreasingInstallment'
  ) {
    return undefined;
  }

  const label =
    alternative.planType === 'increasingInstallment' ? 'Artış' : 'Azalış';

  return `${label} varsayımı: %${
    alternative.installmentIncreaseRatePercent ?? DEFAULT_PROGRESSIVE_RATE_PERCENT
  }, ${alternative.installmentIncreaseFrequencyMonths ?? DEFAULT_PROGRESSIVE_FREQUENCY_MONTHS} ayda bir, ${
    alternative.installmentIncreaseStartNo ?? 1
  }-${alternative.installmentIncreaseEndNo ?? alternative.input.term}. taksit aralığında.`;
};

const buildAlternativeDetails = (
  baseline: LoanCalculationResult,
  alternative: LoanCalculationResult,
  comparison: PaymentPlanRecommendation['comparison']
): string[] => {
  const details = [
  ...
  details.push(
    `Karşılaştırma mevcut ${PLAN_LABELS[baseline.planType]} sonucuna göre yapılmıştır.`
  );

  return details;
};

const createEqualPrincipalRecommendation = (
  baseline: LoanCalculationResult,
  alternative: LoanCalculationResult
...
const createIncreasingInstallmentRecommendation = (
  baseline: LoanCalculationResult,
  alternative: LoanCalculationResult
): RankedRecommendation | undefined => {
  const comparison = buildComparison(baseline, alternative);
...
const createDecreasingInstallmentRecommendation = (
  baseline: LoanCalculationResult,
  alternative: LoanCalculationResult
): RankedRecommendation | undefined => {
  const comparison = buildComparison(baseline, alternative);
...
const createStandardRecommendation = (
  baseline: LoanCalculationResult,
  alternative: LoanCalculationResult
): RankedRecommendation | undefined => {
