...
import { ActivityIndicator, Alert, Linking, RefreshControl, ScrollView, Share, StyleSheet, Switch, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
import { whatsappPhone } from '../utils/phone';
...
export default function RequestsScreen() {
  ...
  }, [fetchAll, isPremium, session]);
  const refresh = useCallback(async () => {
    if (!session || !isPremium) return;
    setRefreshing(true);
    try { await fetchAll(); }
    catch { Alert.alert('Yenilenemedi', 'Talepler şu anda yenilenemedi. Bağlantınızı kontrol edin.'); }
  ...
  const createLink = async () => {
    if (!phone || !isValidPhoneNumber(phone)) { Alert.alert('Geçerli telefon gerekli', 'Ülke kodunu seçip müşterinizin göreceği telefon numaranızı yazın.'); return; }
    try {
  ...
  const updateStatus = async (item:LoanRequest,status:LoanRequest['status']) => { const next=item.status===status?'new':status; try { await apiRequest(`/v1/me/loan-requests/${item.id}`,{method:'PATCH',token:session.token,body:{status:next}}); setRequests(v=>v.map(x=>x.id===item.id?{...x,status:next}:x)); } catch { Alert.alert('Güncellenemedi','Talep durumu güncellenemedi. Bağlantınızı kontrol edip tekrar deneyin.'); } };
  ...
  return <SafeAreaView style={styles.root} edges={['top','left','right']}>
    <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} stickyHeaderIndices={[1]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>void refresh()} tintColor={colors.primary} colors={[colors.primary]}/>}>
